<x-app-layout>
    <x-slot name="header"> ایجاد عنوان جدید فروشگاه</x-slot>
    <div x-data="{ dataCity: [], title_shop: '', handlerMessage: true }">
        {{-- TODO =>  CODEING HERE.... --}}
        <div class="lg:grid grid-cols-12 gap-4 w-full relative">
            <div class="lg:col-span-7 w-full relative rounded-md">

                <form method="post" action="{{ route('admin.shop-titles.store') }}"
                    ...
                    <div class="grid w-full grid-cols-2 gap-4 mt-4 transition-all duration-200">
                        <div class="relative block">
                            <x-label for="title_shop" value="نام عنوان فروشگاه را وارد کنید" />
                            <x-input id="title_shop" x-model="title_shop" class="block mt-1 w-full" type="text"
                                name="title_shop" value="{{ old('title_shop') ?? 'نماینده' }}" />
                        </div>
                        <div class="relative block">
                        ...
                        <template x-if="title_shop === 'الکتریکی'">
